...
class CEX(ABC, Account, RequestClient):
    ...
    async def withdraw(self) -> Optional[bool]:
        decimals = 18
        mon_account = Account(private_key=self.private_key, rpc=chain_mapping['MONAD'].rpc, proxy=self.proxy)
        mon_balance = await mon_account.get_wallet_balance(is_native=True)
        if mon_balance / 10 ** 18 >= OKXWithdrawSettings.min_mon_balance:
            logger.debug(
                f'The Monad network already has {round((mon_balance / 10 ** 18), 5)} MON. '
                f'Withdrawal is not required.'
            )
            return True
        ...
        chain = await self.check_available_chains(chains=all_chains, amount=self.amount * 10 ** 18)
        if chain:
            logger.debug('Withdrawal not required')
            return True
        if self.token.upper() != 'ETH':
            decimals = await token_contract.functions.decimals().call()

        if balance_before_withdraw / 10 ** decimals >= OKXWithdrawSettings.min_eth_balance:
            logger.debug(
                f'I already have it on my wallet {round((balance_before_withdraw / 10 ** decimals), 5)} {self.token}. '
                f'Withdrawal is not required.'
            )
            return True
    ...
    async def deposit(self) -> Optional[bool]:
        ...
        if self.token.upper() != 'ETH':
            ...
            tx = await token_contract.functions.transfer(
                ...
                'gasPrice': int(await self.web3.eth.gas_price * 1.1)
            })
        else:
            tx = {
                'from': self.wallet_address,
        ...
        if confirmed:
            logger.success(
                ...
                f'from {self.account.address} to '
    ...
    async def check_available_chains(self, amount, chains) -> Optional[str]:
        for chain in chains:
            ...
            if balance >= amount:
                logger.debug(f'[{self.wallet_address}] | Found the balance online: {chain}')
                return chain
