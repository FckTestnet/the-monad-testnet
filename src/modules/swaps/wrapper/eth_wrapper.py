...
class Wrapper(Account):
    ...
    def __repr__(self) -> str:
        return f'[{self.wallet_address}] | Wrapper | {"MON => WMON" if self.action.lower() == "wrap" else "WMON => MON"}'

    @retry(retries=RETRIES, delay=PAUSE_BETWEEN_RETRIES, backoff=1.5)
    async def wrap(self) -> None:
        is_native = self.action.upper() == 'WRAP'
        balance = await self.get_wallet_balance(
            is_native=is_native,
            address=tokens['MONAD']['WMON']
        )

        if balance == 0:
            logger.warning(f"🅾️ | Your balance is 0 | {self.wallet_address}")
            return
        ...
        if self.use_all_balance is True:
            if self.action.lower() == 'unwrap':
                ...
                logger.error(f'You can not use use_all_balance = True to wrap MON. Using amount_from, amount_to.')
        if self.use_percentage:
            amount = int(balance * self.percentage_to_wrap)

        tx = await create_wrap_tx(self.wallet_address, self.token, self.web3, amount)
        ...
        if confirmed:
            logger.success(
                f'Successfully {"unwrapped" if self.action.lower() == "unwrap" else "wrapped"} {amount / 10 ** 18} {"WMON => MON" if self.action.lower() == "unwrap" else "MON => WMON"} | TX: https://testnet.monadexplorer.com/tx/{tx_hash}'
            )
        else:
